// Skills Matrix Visualization with Resume Type Support
let currentSkillCategories = [];

// Initialize the skills matrix
function initSkillsMatrix() {
    // Check if there's resume skills data waiting to be displayed
    if (window.resumeSkillsData) {
        currentSkillCategories = window.resumeSkillsData;
        delete window.resumeSkillsData;
    } else {
        // Default skill categories (Product Management focus)
...
// Update the skills matrix with new categories
function updateSkillsMatrix(skillCategories) {
    if (!skillCategories) return;
    
    // Update current categories
    currentSkillCategories = skillCategories;
    
    // Re-render the skills matrix
    renderSkillsMatrix();
}

// Render the skills matrix based on current categories
function renderSkillsMatrix() {
    ...
    currentSkillCategories.forEach((category, index) => {
        // Create tab
...
// Export functions for use in resume-switcher.js
window.updateSkillsMatrix = updateSkillsMatrix;
